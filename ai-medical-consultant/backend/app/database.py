...
def _seed_consult_symptom_presets(table_names: set[str]) -> None:
    if "consult_symptom_presets" not in table_names:
        return
    presets = [
        ("surface", 2, "表证采集", "表虚/表实", "tone-blue", "表虚", 1, ["恶风", "汗出", "自汗", "发热", "头痛", "鼻鸣", "干呕", "脉浮缓", "脉浮弱", "项背不舒", "身体痒", "啬啬恶寒"]),
        ("surface", 2, "表证采集", "表虚/表实", "tone-green", "表实", 2, ["恶寒", "发热", "无汗", "头痛", "身痛", "骨节疼痛", "项背强", "喘", "咳嗽", "脉浮紧", "脉浮数", "不得平卧"]),
        ("interior", 3, "里证采集", "里热/里寒/里虚/里实", "tone-amber", "里热", 1, ["口渴喜冷", "口苦", "烦躁", "便秘", "大便臭秽", "尿赤", "舌红苔黄", "蒸蒸发热", "反酸", "肛门灼热"]),
        ("interior", 3, "里证采集", "里热/里寒/里虚/里实", "tone-blue", "里寒", 2, ["畏寒", "喜热饮", "口不渴", "下利清谷", "腹痛喜温", "四肢厥冷", "舌淡苔白", "呕吐清水", "涎沫", "小便清长"]),
        ("interior", 3, "里证采集", "里热/里寒/里虚/里实", "tone-green", "里虚", 3, ["食少纳呆", "乏力", "自汗", "懒言", "面色萎黄", "不欲食", "腹软喜按", "便溏", "心悸", "脉弱"]),
        ("interior", 3, "里证采集", "里热/里寒/里虚/里实", "tone-red", "里实", 4, ["腹满痛", "便秘拒按", "潮热谵语", "大便硬", "舌苔厚腻", "按之疼痛", "嗳气腐臭", "心下痞硬", "不大便", "脉沉实"]),
        ("half", 4, "半证采集", "半热", "tone-blue", "半热", 1, ["胸胁苦满", "往来寒热", "口苦", "咽干", "目眩", "心烦", "默默不欲饮食", "喜呕", "胸闷", "低热", "胸胁不舒", "脉弦"]),
        ("water", 5, "水证采集", "水虚/水实", "tone-green", "水虚", 1, ["口干不欲饮", "小便清长", "夜尿频", "水肿按之陷", "眩晕", "消瘦", "腰膝酸软", "畏寒", "脉沉细"]),
        ("water", 5, "水证采集", "水虚/水实", "tone-amber", "水实", 2, ["小便不利", "心下悸", "痰多", "水肿", "呕吐清水", "身重", "饮水即胀", "咳喘痰白", "头眩", "脉沉滑"]),
        ("qi", 6, "气证采集", "气虚/气实", "tone-green", "气虚", 1, ["短气乏力", "声低", "懒言", "自汗", "动则喘", "面色白", "食欲差", "脉弱", "倦怠"]),
        ("qi", 6, "气证采集", "气虚/气实", "tone-amber", "气实", 2, ["胀满", "嗳气", "气上冲", "胸闷", "咽中如有物", "叹息", "情绪加重", "胁胀", "胸满", "脉弦"]),
        ("blood", 7, "血证采集", "血虚/血实", "tone-green", "血虚", 1, ["面色萎黄", "心悸失眠", "头晕", "经量少", "爪甲不荣", "唇色淡", "乏力", "脉细", "肌肤甲错"]),
        ("blood", 7, "血证采集", "血虚/血实", "tone-red", "血实", 2, ["刺痛固定", "少腹急结", "舌暗紫", "口唇暗", "经血块", "皮下瘀斑", "夜间痛甚", "出血", "脉涩", "拒按"]),
        ("yin", 8, "阴性采集", "阴性证据", "tone-amber", "阴性", 1, ["精神疲惫", "嗜睡", "萎靡", "懒言", "倦怠", "面色白", "声低", "脉微细", "不欲动"]),
    ]
    with engine.begin() as conn:
