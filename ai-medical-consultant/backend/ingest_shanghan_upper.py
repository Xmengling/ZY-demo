# -*- coding: utf-8 -*-
"""将《伤寒论-上册》讲稿按「第 N 条」切片导入知识库（一条文一片段）。"""
...
SOURCE_FILE = Path(
    r"D:\AI\ZY-Study\data\《伤寒论-上册》胡希恕、李冠杰讲稿合订本-原文标注.docx"
)
SOURCE_NAME = SOURCE_FILE.name
CATEGORY = "伤寒论讲稿"
DEPARTMENT = "胡希恕·李冠杰讲稿"
# 仅匹配条文标题行且紧跟「原文：」，避免讲解文中「第 N 条从…」被误切
...
def _article_title(piece: str, index: int) -> str:
    ...
    if m:
        return f"伤寒论上册·第{m.group(1)}条"
    return f"伤寒论上册·片段{index}"
