/**
 * 伤寒论读书课堂流式聊天（SSE）
 * @param {object} data - { message, articleNo, mode, state }
 * @param {object} handlers - { onStart, onToken, onDone, onError }
 * @param {AbortSignal} [signal]
 */
export async function shanghanStudyChatStream(data, handlers = {}, signal) {
  const token = localStorage.getItem('token')
  const res = await fetch('/api/v1/shanghan/study/chat/stream', {
    method: 'POST',
    headers: {
